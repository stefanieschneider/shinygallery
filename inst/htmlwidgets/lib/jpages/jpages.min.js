! function(t, i, s, e) {
    ...
    function r(e, n) {
        this.options = t.extend({}, h, n), this._container = t(this.options.container + " .gallery-container"), this._container.length && (this.jQwindow = t(i), this.jQdocument = t(s), this._holder = t(e), this._nav = {}, this._first = t(this.options.first), this._previous = t(this.options.previous), this._next = t(this.options.next), this._last = t(this.options.last), this._items = this.options.items, this._itemsShowing = t([]), this._itemsHiding = t([]), this._numPages = Math.ceil(this._items.length / this.options.perPage), this._currentPageNum = this.options.startPage, this._clicked = !1, this.init())
    }
    r.prototype = {
        constructor: r,
        init: function() {
            this.setNav(), this.paginate(this._currentPageNum)
        },
        setNav: function() {
            var i = this.writeNav();
            this._holder.each(this.bind(function(s, e) {
                var n = t(e);
                n.html(i), this.cacheNavElements(n, s), this.bindNavHandlers(s), this.disableNavSelection(e)
            }, this))
        },
        writeNav: function() {
            var t, i = 1;
            for (t = this.writeBtn("first") + this.writeBtn("previous"); i <= this._numPages; i++) {
                switch (1 === i && 0 === this.options.startRange && (t += "<span>...</span>"), i > this.options.startRange && i <= this._numPages - this.options.endRange ? t += "<a href='#' class='jp-hidden'>" : t += "<a>", this.options.links) {
                    case "numeric":
                        t += i;
                        break;
                    case "blank":
                        break;
                    case "title":
                        var s = this._items.eq(i - 1).attr("data-title");
                        t += void 0 !== s ? s : ""
                }
        ...
        writeBtn: function(i) {
            return !1 === this.options[i] || t(this["_" + i]).length ? "" : "<a class='jp-" + i + "'>" + this.options[i] + "</a>"
        },
        cacheNavElements: function(i, s) {
            this._nav[s] = {}, this._nav[s].holder = i, this._nav[s].first = this._first.length ? this._first : this._nav[s].holder.find("a.jp-first"), this._nav[s].previous = this._previous.length ? this._previous : this._nav[s].holder.find("a.jp-previous"), this._nav[s].next = this._next.length ? this._next : this._nav[s].holder.find("a.jp-next"), this._nav[s].last = this._last.length ? this._last : this._nav[s].holder.find("a.jp-last"), this._nav[s].fstBreak = this._nav[s].holder.find("span:first"), this._nav[s].lstBreak = this._nav[s].holder.find("span:last"), this._nav[s].pages = this._nav[s].holder.find("a").not(".jp-first, .jp-previous, .jp-next, .jp-last"), this._nav[s].permPages = this._nav[s].pages.slice(0, this.options.startRange).add(this._nav[s].pages.slice(this._numPages - this.options.endRange, this._numPages)), this._nav[s].pagesShowing = t([]), this._nav[s].currentPage = t([])
        },
        bindNavHandlers: function(i) {
            var s = this._nav[i];
            s.holder.bind("click.jPages", this.bind(function(i) {
                var e = this.getNewPage(s, t(i.target));
                this.validNewPage(e) && (this._clicked = !0, this.paginate(e)), i.preventDefault()
        ...
        paginate: function(i) {
            var s, e;
            s = this.updateItems(i), e = this.updatePages(i), this._currentPageNum = i, t.isFunction(this.options.callback) && this.callback(i, s, e)
        },
        updateItems: function(i) {
            var s = this.getItemRange(i);
            return this._itemsShowing = this._items.slice(s.start, s.end), this._itemsHiding = t(this._items).not(this._itemsShowing).get(), this.jQAnimations(i), s
        },
        ...
        jQAnimations: function(t) {
            clearInterval(this._delay), this._container.html(this._itemsShowing.join("")), this._itemsOriented = this._itemsShowing, this._index = 0, this._delay = setInterval(this.bind(function() {
                this._index === this._itemsOriented.length ? clearInterval(this._delay) : this._itemsOriented, this._index = this._index + 1
            }, this), this.options.delay)
        },
        updatePages: function(t) {
            var i, s, e;
            for (s in i = this.getInterval(t), this._nav) this._nav.hasOwnProperty(s) && (e = this._nav[s], this.updateBtns(e, t), this.updateCurrentPage(e, t), this.updatePagesShowing(e, i), this.updateBreaks(e, i));
            return i
        },
        getInterval: function(t) {
            var i, s;
            return i = Math.ceil(this.options.midRange / 2), s = this._numPages - this.options.midRange, {
                start: t > i ? Math.max(Math.min(t - i, s), 0) : 0,
                end: t > i ? Math.min(t + i - (this.options.midRange % 2 > 0 ? 1 : 0), this._numPages) : Math.min(this.options.midRange, this._numPages)
            }
        },
        ...
        updateCurrentPage: function(t, i) {
            t.currentPage.removeClass("jp-current"), t.currentPage = t.pages.eq(i - 1).addClass("jp-current")
        },
        updatePagesShowing: function(t, i) {
            var s = t.pages.slice(i.start, i.end).not(t.permPages);
            t.pagesShowing.not(s).addClass("jp-hidden"), s.not(t.pagesShowing).removeClass("jp-hidden"), t.pagesShowing = s
        },
        ...
        destroy: function() {
            this.jQdocument.unbind("keydown.jPages"), this._container.unbind("mousewheel.jPages DOMMouseScroll.jPages"), this._holder.unbind("click.jPages").empty()
        }
    }, t.fn[n] = function(i) {
        var s = t.type(i);
        return "object" === s ? (this.length && !t.data(this, n) && (a = new r(this, i), this.each(function() {
            t.data(this, n, a)
        })), this) : "string" === s && "destroy" === i ? (a.destroy(), this.each(function() {
            t.removeData(this, n)
        }), this) : "number" === s && i % 1 == 0 ? (a.validNewPage(i) && a.paginate(i), this) : this
    }
